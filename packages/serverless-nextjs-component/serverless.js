...
const {
    DEFAULT_LAMBDA_CODE_DIR, API_LAMBDA_CODE_DIR
} = require("./constants");
...
class NextjsComponent extends Component {
    ...
    async readApiBuildManifest(nextConfigPath) {
        const path = join(
        nextConfigPath, ".serverless_nextjs/api-lambda/manifest.json");
        return (await fse.exists(path)) ? fse.readJSON(path) : Promise.resolve(undefined);
    }
    ...
    async build(inputs = {}) {
        const nextConfigPath = inputs.nextConfigDir ? path.resolve(inputs.nextConfigDir) : process.cwd();

        const buildConfig = {
            enabled: inputs.build ? inputs.build !== false && inputs.build.enabled !== false : true,
            cmd: "node_modules/.bin/next",
            args: ["build"],
            ...(typeof inputs.build === "object" ? inputs.build : {}),
            cwd: inputs.build && inputs.build.cwd ? path.resolve(inputs.build.cwd) : nextConfigPath
        };
        ...
        const hasAPIPages = Object.keys(apiBuildManifest.apis.nonDynamic).length > 0 || Object.keys(apiBuildManifest.apis.dynamic).length > 0;
    ...
    async deploy(inputs = {}) {
        ...
        cloudFront,
        defaultEdgeLambda,
        apiEdgeLambda] = await Promise.all([
        ...
        defaultBuildManifest.pages.html.dynamic).map(x = > x.file);
        ...
        const hasAPIPages = apiBuildManifest && (Object.keys(apiBuildManifest.apis.nonDynamic).length > 0 || Object.keys(apiBuildManifest.apis.dynamic).length > 0);

        const getLambdaMemory = lambdaType = > typeof inputs.memory === "number" ? inputs.memory : (inputs.memory && inputs.memory[lambdaType]) || 512;

        const getLambdaTimeout = lambdaType = > typeof inputs.timeout === "number" ? inputs.timeout : (inputs.timeout && inputs.timeout[lambdaType]) || 10;

        const getLambdaName = lambdaType = > typeof inputs.name === "string" ? inputs.name : inputs.name && inputs.name[lambdaType];
            ...
            cloudFrontOrigins[0].pathPatterns["api/*"] = {
                ...
                "lambda@edge": {
                    "origin-request": `$ {
                        ...
                        apiEdgeLambdaPublishOutputs.version
                    }`
                },
            ...
            defaults: {
                ...
                "lambda@edge": {
                    "origin-request": `$ {
                        ...
                        defaultEdgeLambdaPublishOutputs.version
                    }`
                }
            },
